...
public class FileTest {
	...
	public static void main(String[] args) {
		...
		if(!dir2.exists()) {
			System.out.println(dir2.getName()+"폴더가 존재하지 않습니다. 새로운 폴더를 생성합니다.");
			dir2.mkdir();
			System.out.println(dir2.getName()+"폴더가 생성되었습니다.");
		}else {
			System.out.println(dir2.getName()+"폴더가 존재합니다.");
		}
		
		// 파일 존재 확인
		File file1 = new File(dir2, System.currentTimeMillis()+".txt");
		System.out.println(dir2.getName()+"폴더에 "+file1.getName() +"파일 존재 유무: " + file1.exists());
		System.out.println(file1);
		
		System.out.println();
		
		System.out.println("===== "+dir1.getPath()+" 폴더의 파일 리스트 =====");
		...
		File deleteFile = new File(dir1, "testcopy1.exe");
		System.out.println(deleteFile.getParent()+"폴더의 "+deleteFile.getName()+" 파일 존재 여부: "+deleteFile.exists());
		...
		System.out.println("파일 존재 여부: "+deleteFile.exists());
		
		System.out.println();
		System.out.println("===== test 폴더의 모든 파일을 backup폴더로 이동 =====");

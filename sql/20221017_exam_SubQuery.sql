...
from  emp a, (select  deptno, min(sal) as minsal  from emp where deptno in (select distinct deptno from dept)  group by deptno) b
...
select ename from emp where empno not in (select empno from emp where empno in (select mgr from emp));
...
select empno, ename, sal from emp where sal > (select avg(sal) from emp) and deptno in (select distinct deptno from emp where ename like '%M%');

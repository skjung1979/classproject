...
select ename, sal, sal+300 as add_Sal from emp order by 3;
-- order by에 컬럼명 대신 숫자로 컬럼을 지정할 수 있다. 하지만 별칭을 주어 order by 하는 것을 권장한다.

-- 2. 사원의 이름, 급여, 연간 총 수입을 총 수입이 많은 것부터 작은 순으로 출력하시오, 연간 총수입은 월급에 12를 곱한 후 $100의 상여금을 더해서 계산하시오.
select ename, sal, sal*12+100 as year_sal from emp order by year_sal desc;
